  ...
  <div>
    ...
    <base-card>
      <form @submit.prevent="submitForm">
        ...
        <base-button type="button" mode="flat" @click="switchAuthMode"> {{ switchModeButtonText }}</base-button>
      </form>
    </base-card>
  </div>
</template>
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        if (this.mode === 'login') {
          await this.$store.dispatch('login', formPayload);
        } else {
          await this.$store.dispatch('signup', formPayload);
        }
